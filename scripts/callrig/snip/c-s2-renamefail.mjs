export default async ({page}) => {
  ...
  const made=await page.evaluate(async (ws)=>{
    const name='qa-c2-rnf-'+Math.random().toString(36).slice(2,6);
  ...
  const nameIn=inputs.first();
  await nameIn.fill('qa-c2-renamed-probe');
  await page.waitForTimeout(900);
  out.typed=await nameIn.inputValue();
  ...
  out.after=await page.evaluate(async (id)=>{
    ...
    const inp=[...document.querySelectorAll('input')].filter(v)[0];
    const r=await fetch(`/api/v1/channels/${id}`,{credentials:'include'});
    let j=null; try{j=await r.json()}catch{}
    return {fieldValue:inp?inp.value.slice(0,26):'no input',
