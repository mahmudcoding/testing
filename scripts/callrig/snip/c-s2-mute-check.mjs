  ...
  return await page.evaluate(async ()=>{
    ...
    for (let i=0;i<6;i++){
      ...
      out.push({ms:Date.now()-t0, total:j.total,
        plainHit:/MUTE-PLAIN|MUTE\\-PLAIN/.test(blob), mentionHit:/MUTE-MENTION|MUTE\\-MENTION/.test(blob),
        newest:{t:l[0]&&l[0].type, ti:(l[0]&&l[0].title||'').slice(0,26), b:(l[0]&&(l[0].body||'')||'').slice(0,40)}});
      const last=out[out.length-1];
      if (last.plainHit && last.mentionHit) break;
