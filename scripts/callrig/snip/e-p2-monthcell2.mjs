...
export default async ({page}) => {
  ...
  // click "Next" twice so the trailing cells are in a month with no events
  for(let i=0;i<2;i++){
  ...
  const target = await page.evaluate(`(() => { ${VISFN}
     ...
     // choose a trailing cell that contains NO event chip
     const empty=trailing.filter(e=>{const c=e.closest('[data-testid="calendar-month-cell"]');
       return c && !c.querySelector('[data-testid="calendar-month-event-chip"],[data-testid="calendar-event-chip"]');});
     const pick=empty[empty.length-1]; if(!pick) return null;
  ...
  out.after = await page.evaluate(`(() => { ${boxVis}
     ...
     return {dialogOpen:true, fields:f,
             summary:(t.match(/\\w{3}, \\w{3} \\d{1,2}[^|]{0,34}/)||[''])[0], head:t.slice(0,60)}; })()`);
  out.startMatchesClickedDate = out.after.fields && out.after.fields['event-start']===target.date;
