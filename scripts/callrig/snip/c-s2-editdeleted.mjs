export default async ({page}) => {
  ...
  const tag='QA-EDITDEL-'+Math.random().toString(36).slice(2,5);
  ...
  out.editModeText=await comp.evaluate(e=>e.innerText.trim().slice(0,24));
  await comp.click(); await page.keyboard.press('Meta+A');
  await page.keyboard.type(tag+' EDITED');
  await page.waitForTimeout(800);
  // now delete the message out from under the open editor
  out.deleteStatus=await page.evaluate(async ({ch,id})=>{
    const r=await fetch(`/api/v1/messaging/channels/${ch}/messages/${id}`,
      {method:'DELETE',credentials:'include'});
    return r.status;},{ch,id:seed});
  ...
  const poll=(async()=>{const t0=Date.now();
    while(Date.now()-t0<10000){
      ...
      t.forEach(x=>toasts.add(x.slice(0,70)));
