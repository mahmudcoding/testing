...
export default async ({page}) => {
  ...
  out.mdBtn=await md.count();
  out.mdBefore=out.mdBtn? await md.first().getAttribute('aria-pressed'):null;
  if(out.mdBtn){ await md.first().click(); await page.waitForTimeout(1000); }
  out.mdAfter=out.mdBtn? await md.first().getAttribute('aria-pressed'):null;
  if(!await empty(page, comp)) return {...out, err:'not empty'};
  await comp.type('/me QA-S2-MEMD waves in markdown', {delay:40});
  await page.waitForTimeout(1300);
  out.before=await comp.evaluate(e=>e.innerText);
  const posts=[];
  ...
  out.afterEnter1=await comp.evaluate(e=>e.innerText);
  ...
  out.rendered = await page.evaluate(()=>{
    const el=[...document.querySelectorAll('main [data-message-id]')].reverse()
      .find(e=>/QA-S2-MEMD/.test(e.innerText||''));
    return el? {t:(el.innerText||'').replace(/\s+/g,' ').slice(0,70),
      em:el.querySelectorAll('em,i').length, strong:el.querySelectorAll('strong,b').length}:'not found';
  });
  // restore markdown mode
  if(out.mdBtn){ await md.first().click(); await page.waitForTimeout(800);
