...
const ROUTES = ['settings/admin/audit-log'];
...
export default async ({ page, ctx }) => {
  ...
  for (const r of ROUTES) {
    ...
    let same=0, diff=0; const samples=[], stuck=[];
    for (let i=0;i<n;i++){
      if (a[i]===b[i]) { same++; if (stuck.length<12) stuck.push(a[i]); }
      else { diff++; if (samples.length<2) samples.push({ tashkent:a[i], newYork:b[i] }); }
    }
    report[r]={ found:a.length, alsoFound:b.length, unchanged:same, changed:diff, samples, stuck };
  }
