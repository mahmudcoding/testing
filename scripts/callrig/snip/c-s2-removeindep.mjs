export default async ({page}) => {
  const ws='W4QCF1XTURESO01', carol='U4QCCAROL000001';
  const made=await page.evaluate(async ({ws,carol})=>{
    const name='qa-c2-rm-'+Math.random().toString(36).slice(2,6);
    ...
    const a=await fetch('/api/v1/channels/members/add',{method:'POST',credentials:'include',
      headers:{'content-type':'application/json'},
      body:JSON.stringify({channel_id:id,user_id:carol})});
    return {id,name,add:a.status};
  },{ws,carol});
  ...
  let click='no';
  try { await page.locator('button[aria-label="Remove QA Carol"]').first().click({timeout:6000}); click='ok'; }
  ...
  const res=await page.evaluate(async (id)=>{
    ...
    return {landed:window.__c, members:Array.isArray(a)?a.length:null,
      ...
      toasts:[...document.querySelectorAll('[role="status"],[role="alert"]')].filter(v)
        .map(e=>(e.innerText||'').trim().slice(0,40)).filter(Boolean)};}, made.id);
  await page.evaluate(async (id)=>{await fetch(`/api/v1/channels/${id}/archive`,{method:'POST',credentials:'include'});}, made.id);
  return {channel:made.name, addStatus:made.add, click, apiCallsAfterClick:reqs.length,
          calls:reqs.slice(0,3), ...res};
};
