export default async ({page}) => {
  ...
  const picked = await page.evaluate(({v,who})=>{ const vis=eval(v);
    ...
    if(!t) return {notFound:true, rows:cbs.map(c=>{let r=c; for(let k=0;k<5&&r.parentElement;k++){r=r.parentElement; if((r.innerText||'').trim().length>2) break;} return (r.innerText||'').trim().slice(0,20);})};
    const was=t.checked;
