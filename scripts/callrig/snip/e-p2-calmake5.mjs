...
export default async ({page}) => {
  ...
  await page.locator('input[data-field="event-title"]').first().fill('QA-E RSVP three');
  await page.locator('input[data-field="event-start-time"]').first().fill('22:00');
  await page.waitForTimeout(500);
  await page.locator('input[data-field="event-end-time"]').first().fill('22:30');
  await page.waitForTimeout(700);
  const sm = page.locator('input[placeholder="Search members"]').first();
  await sm.scrollIntoViewIfNeeded(); await sm.fill('Bob');
  await page.waitForTimeout(2200);
  out.picked = await page.evaluate(`(() => { ${VISFN} ${CLICKDEEPEST} ${boxVisFn}
  ...
  out.selectedState = await page.evaluate(`(() => { ${boxVisFn}
    ...
    return m? m[0].replace(/\\n+/g,' ').slice(0,70) : '(none) tail='+(d.innerText||'').replace(/\\n+/g,' ').slice(-100); })()`);
  writes.length=0;
