export default async ({page, ctx}) => {
  ...
  const seed=await page.evaluate(async (ch)=>{
    const r=await fetch('/api/v1/messaging/messages',{method:'POST',credentials:'include',
      ...
      body:JSON.stringify({channel_id:ch, body:'@qa_c_bob QA-COPY2 check',
                           mention_user_ids:['U4QCBOB00000001']})});
  ...
  out.onScreen=await page.evaluate((id)=>{
    ...
    return e?(e.innerText||'').replace(/\s+/g,' ').trim().slice(-44):'gone';}, seed);
  out.stored=await page.evaluate(async (id)=>{
    ...
    return hit?{body:hit.body, mentions:JSON.stringify(hit.mention_ids||hit.mentions||[]).slice(0,40)}:'gone';}, seed);
  ...
  await msg.locator('button[aria-label="Copy text"]').first().click({timeout:6000}).catch(()=>{out.err='FAIL';});
  await page.waitForTimeout(3500);
  out.landed=await page.evaluate(()=>window.__c);
  out.clipboard=await page.evaluate(async ()=>{
    try { return (await navigator.clipboard.readText()).slice(0,90); } catch(e){ return 'READ-FAILED'; }});
  return out;
};
