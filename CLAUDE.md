...
- Throttling comes from **`twg`, not from Jira**. Atlassian's own headers on a REST call show `"global-app-quota";q=65000;w=3600` and `"jira-burst-based";q=100;w=1` — 65k requests an hour, 100/s burst, so a 35-page sync is a rounding error against it. What actually fails is `TWG_RATE_LIMITED` (429) from twg's own gateway: it tripped after a handful of rapid queries and stayed down ~25 minutes, while the Atlassian MCP kept working throughout — a separate bucket. Its size is not published; pacing requests ~1.5 s apart has gone through untouched, and `jira_cache.py` backs off and retries on it. `twg doctor` reports "Token: valid" while throttled, and `twg --output-file` writes `{"ok":false,"error":{"code":"TWG_RATE_LIMITED"}}` into the file while still printing a normal `stdout=<path>` line; check the payload, not the exit path. `twg api <path> -i` prints the response headers if you need to see the live quota.
