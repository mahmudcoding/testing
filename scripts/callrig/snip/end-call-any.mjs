export default async ({page}) => {
  ...
  const res = await page.evaluate(async(id)=>{const r=await fetch(`/api/v1/meeting/${id}/end`,{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:'{}'});return r.status+' '+(await r.text()).slice(0,100);}, cur);
