...
### Заведено в Jira по прямой просьбе пользователя — ALK-3761

`[BE][CALLS] Гость, потерявший связь из side room, навсегда занимает место в звонке —
и живого человека туда уже не пускают` → **ALK-3761**, Bug / Backlog / High / `backend`,
https://ttbrm.atlassian.net/browse/ALK-3761

Перед заведением: `jira_cache.py sync` (+2 тикета, ALK-3759 и ALK-3760 — оба [FE-WEB][SIDE ROOMS]
про другое), повторный дедуп по открытым багам со словами side room / guest / место / лимит —
совпадений нет. Ближайшие соседи: **ALK-2721** (гостю после перезагрузки показывают неверную
причину выселения) и **ALK-2871** (гость висит на Waiting for approval после завершения звонка) —
оба про то, что гость *видит*, ни один не про удерживаемое место и не про отказ живому человеку.

Тело собрано из статьи отчёта, блок «Воспроизведение» со snippet'ом снят (это стендовая деталь),
пробел-скан на утечки чист. Проверено после создания: все пять секций и measurement-блок на месте.

### Заведено в Jira по прямой просьбе пользователя — ALK-3762

`[BE][CALLS] Ожидающий перезагрузил страницу — ведущий впускает его, но в звонок никто не приходит`
→ **ALK-3762**, Bug / Backlog / Medium / `backend`, https://ttbrm.atlassian.net/browse/ALK-3762

Дедуп: прочитаны целиком два кандидата, оба не владеют находкой по своим критериям приёмки.
**ALK-3250** — счётчик Waiting Lobby показывает 0 другим участникам; вся его «Проверка» про
синхронизацию списка и счётчика, не про доставку впуска. **ALK-2871** — гость залипает на
Waiting for approval, когда ведущий **завершает** звонок; все acceptance checks про переход в
terminal state «Call ended». У меня звонок живой, ведущий жмёт Admit, и человек не приходит —
другой триггер, непересекающиеся критерии.

Тело — из статьи отчёта, блок «Воспроизведение» снят, утечек нет. «Подтверждённая причина»
сохранена: она держится на положительном контроле (гость по ссылке-приглашению перезагрузку
переживает, и Admit его вносит), то есть расходятся два соседних пути, а не «фича сломана».
