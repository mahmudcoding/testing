...
export default async ({page}) => {
  ...
  await dlg.locator('input[aria-label="Add title"]').fill('QA-E RSVP two');
  await dlg.locator('input[aria-label="Starts time"]').fill('21:00');
  await page.waitForTimeout(500);
  await dlg.locator('input[aria-label="Ends time"]').fill('21:30');
  await page.waitForTimeout(500);
  // search the member picker
  await dlg.locator('input[aria-label="Search members"]').fill('Bob');
  ...
  const picked = await page.evaluate(`(() => { ${VISFN} ${boxVisFn}
    const d=[...document.querySelectorAll('[role=dialog]')].filter(boxVis).pop();
  ...
  out.selectedState = await page.evaluate(`(() => { ${boxVisFn}
    const d=[...document.querySelectorAll('[role=dialog]')].filter(boxVis).pop();
    return (d.innerText.match(/Selected \\(\\d+\\)[^|]{0,60}/)||['(no Selected marker)'])[0].replace(/\\n+/g,' ').slice(0,80); })()`);
  writes.length=0;
