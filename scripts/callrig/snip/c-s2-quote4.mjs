export default async ({page}) => {
  ...
  await replyA.scrollIntoViewIfNeeded().catch(()=>{});
  await replyA.hover(); await page.waitForTimeout(900);
  await replyA.locator('button[aria-label="Reply here"]').first().click({timeout:10000});
  ...
  out.chip = await page.evaluate(()=>{
    ...
    return {text: box? box.innerText.replace(/\n+/g,' | ').slice(0,200):null,
      quoteButtons: box? [...box.querySelectorAll('button')].filter(vis).map(b=>(b.getAttribute('aria-label')||'').trim()).filter(t=>/quote|clear/i.test(t)):[]};
  });
