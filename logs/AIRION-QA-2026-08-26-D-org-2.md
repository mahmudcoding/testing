...
## Заведено в Jira (по прямой просьбе пользователя, 27.08)

До этого момента в прогоне ничего не заводилось — по правилу CLAUDE.md «filing is a separate
decision the user makes». Пользователь назвал четыре находки поимённо и попросил их завести.

| тикет | находка | приоритет |
|---|---|---|
| [ALK-3763](https://ttbrm.atlassian.net/browse/ALK-3763) | Право приглашать в workspace открывает страницу, на которой неактивны все элементы управления | High |
| [ALK-3764](https://ttbrm.atlassian.net/browse/ALK-3764) | Право «View the company audit log» не открывает журнал аудита | High |
| [ALK-3765](https://ttbrm.atlassian.net/browse/ALK-3765) | Право создавать роли выдано, но экран, на котором роли создаются, не открывается | High |
| [ALK-3766](https://ttbrm.atlassian.net/browse/ALK-3766) | Блок хранилища подписан «My storage», хотя строкой ниже сказано, что хранилище общее | Low |
| [ALK-3767](https://ttbrm.atlassian.net/browse/ALK-3767) | Приглашение, отправленное без роли, выводится как Role unavailable | Medium (по указанию пользователя; в отчёте Low) |

Все четыре: `issuetype = Bug`, label `frontend`, исполнитель по умолчанию
«TBM - Dasturiy taʼminot», статус `Backlog`.

**Перед заведением** зеркало ALK досинхронизировано (`+12 → 3760 issues`) и дедуп перепроверен
на свежих данных. Ни одна из четырёх не совпала с открытым багом. Ближайшие соседи названы
внутри тел тикетов, а не выброшены:

- ALK-3763 — регресс закрытой **ALK-2965** (`TESTING`, то есть закрыта): её ожидаемый результат
  описывает ровно это, починена была половина про состояния ошибки, а не про доступность действия.
- ALK-3764 — рядом **ALK-3535** (`REVIEW`, та же страница, но про состав списка и другое место
  в коде) и **ALK-3307** (Task про будущую страницу журнала компании); обе разобраны в теле.
- ALK-3765 — прецедент **ALK-3000**, закрытый, тот же дефект на соседнем праве `member.kick`,
  с рассуждением, оставшимся в коде тремя строками выше.
- ALK-3766 — оставшаяся половина **ALK-2654** (`REVIEW`).

**Проверено после заведения:** тело ALK-3765 разобрано на ADF корректно — 7 заголовков,
12 абзацев, 4 блока кода, нумерованный и маркированный списки; поля `Bug` / `High` /
`frontend` / исполнитель проставлены.

Плейсхолдеры (`<CO>`, `<WS>`, `<ID>`, `<TOKEN>`) в телах восстановлены вручную: при извлечении
текста из HTML-отчёта регулярка по тегам съедала их как разметку, и в сыром виде измерения
читались бы как `company..role.manage` и `/api/v1/companies//roles`.

- ALK-3767 — вариант «без роли» появился как штатный выбор формы в **ALK-3157**, поэтому у него
  должна быть своя формулировка; текущий текст совпадает с текстом про нечитаемые роли.

Приоритет ALK-3767 выставлен **Medium** по прямому указанию пользователя — в отчёте эта находка
оценена как Low.

Остальные находки отчёта **не заведены** — заводились только названные.
