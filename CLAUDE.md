...
- `scripts/` — helper scripts. `jira_cache.py` mirrors the whole ALK project locally so ticket lookups cost no Jira calls (see Reporting); `jira.py` does Jira reads and writes through `twg`; `callrig/` drives real WebRTC calls (see "Call testing rig").
...
4. `scripts/jira_cache.py sync` — mirrors every ALK ticket (all statuses and types, with descriptions) to `~/.cache/aloqa-qa/`. First run is a full fetch (~3.5 min, 35 paced pages); later runs top up by `updated >=` in about a second. Everything after that — `show`, `list`, `grep` — is local and free.
...
- **Dedup against Jira before reporting.** Don't file or report anything already covered by an open ALK ticket. Scope the check to **every status that is not Done** — `TESTING` is the team's "merged, awaiting QA" queue and holds the bulk of the project's bugs, so a check limited to Backlog/Ready/In Progress misses most of them.
- Dedup by reading, not searching: `jira_cache.py list --type Bug` and read every summary, then `jira_cache.py show <KEY>` for the full description of anything even loosely related. Titles and descriptions diverge often enough that keyword and stem matching misses real duplicates; `grep` is for spot-checks only. All of it reads the local mirror.
- Jira throttling is about **burst rate, not volume**: a handful of rapid queries trips a ~25-minute cooldown, while 35 pages paced ~1.5 s apart go through untouched — which is why the full mirror is affordable. `twg doctor` reports "Token: valid" while throttled, and `twg --output-file` writes `{"ok":false,"error":{"code":"TWG_RATE_LIMITED"}}` into the file while still printing a normal `stdout=<path>` line; check the payload, not the exit path.
