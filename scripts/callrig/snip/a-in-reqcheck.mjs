export default async ({page}) => {
  return await page.evaluate(()=>{
    ...
    const hits=[...r.querySelectorAll('*')].filter(e=>{
      ...
      return /request|wants to share|asking/i.test(own) || /request/i.test(tid);
    }).map(e=>{const q=e.getBoundingClientRect();
      return ((e.getAttribute&&e.getAttribute('data-testid'))||'')+'|'+(e.textContent||'').trim().slice(0,44)+'|'+Math.round(q.width)+'x'+Math.round(q.height);});
    const p=document.querySelector('[data-testid="call-side-panel-slot"]');
    return {hits:hits.slice(0,6), panel:p?(p.innerText||'').replace(/\s+/g,' ').slice(0,220):'(no panel)'};
  });
};
