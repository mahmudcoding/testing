export default async ({ page }) => {
  const who = process.env.QA_DENY || 'QA Bob';
  const net = [];
  page.on('response', r => { if (/deny|reject/i.test(r.url())) net.push({st:r.status(), u:r.url().slice(-60)}); });
  const clicked = await page.evaluate((who) => {
    const v = el => { const r=el.getBoundingClientRect(); return r.width>0&&r.height>0; };
    const btns = [...document.querySelectorAll('button')].filter(b=>/^Deny$/.test(b.textContent.trim()) && v(b));
    for (const b of btns) {
      let n=b, h=0;
