...
- `scripts/` — helper scripts. `jira_cache.py` mirrors the whole ALK project locally so ticket lookups cost no Jira calls (see Reporting); `jira.py` files and comments; `jira_api.py` is the shared transport both use; `callrig/` drives real WebRTC calls (see "Call testing rig").
...
- Jira goes through `scripts/jira_api.py`: **direct REST with a personal API token** when `QA_JIRA_TOKEN` is in `seed/.env.local`, and `twg` when it is not. A personal token is subject only to Atlassian's `"jira-burst-based";q=100;w=1` — 100 requests a second, no app quota — so throttling is effectively off the table; `scripts/jira_token_check.sh` confirms auth and prints the live quota. `twg` has its own tighter, unpublished limiter: it trips after a handful of rapid queries and stays down ~25 minutes, reports "Token: valid" from `twg doctor` while throttled, and writes `{"ok":false,"error":{"code":"TWG_RATE_LIMITED"}}` into `--output-file` while still printing a normal `stdout=<path>` line — check the payload, not the exit path. The sync paces itself only on the twg path.
