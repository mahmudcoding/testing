...
const MEAS = `(() => { const vis=(VISFN);
  ...
  for (const el of leaves) {
    ...
    const bg=effBg(el);
    // composite a semi-transparent colour over its real background before scoring;
    // scoring rgba(255,255,255,0.36) as pure white silently passes failing text
    const fgc = fg.a>=0.999 ? fg : { r: fg.a*fg.r+(1-fg.a)*bg.r,
...
export default async ({ page }) => {
  ...
  const acc={ light:{ total:0, checked:0, byColour:{}, perRoute:{} },
              dark: { total:0, checked:0, byColour:{}, perRoute:{} } };
  try {
    for (const PATH of ROUTES) {
      await page.goto(`https://airion-cargo.store/w/${W}/settings/${PATH}`, { waitUntil:'networkidle' });
      for (const theme of ['light','dark']) {
        await page.emulateMedia({ colorScheme: theme });
        await page.waitForTimeout(theme==='light' ? 2000 : 1500);
        const res = await page.evaluate(meas);
        const a=acc[theme];
        a.perRoute[PATH]=res.failCount; a.total+=res.failCount; a.checked+=res.checked;
        for (const f of res.fails) { const k=f.fg+' on '+f.bg;
          if(!a.byColour[k]) a.byColour[k]={n:0,worst:99};
          a.byColour[k].n++; a.byColour[k].worst=Math.min(a.byColour[k].worst,f.ratio); }
      }
    }
  } finally { await page.emulateMedia({ colorScheme:null }); }
  const shape=a=>({ leavesChecked:a.checked, totalFailures:a.total,
    routesClean:Object.values(a.perRoute).filter(x=>x===0).length,
    worstRoute:Object.entries(a.perRoute).sort((x,y)=>y[1]-x[1])[0],
    byColour:Object.entries(a.byColour).map(([k,v])=>({colour:k,count:v.n,worst:v.worst}))
      .sort((x,y)=>y.count-x.count).slice(0,4) });
  return { routes:ROUTES.length, light:shape(acc.light), dark:shape(acc.dark) };
};
